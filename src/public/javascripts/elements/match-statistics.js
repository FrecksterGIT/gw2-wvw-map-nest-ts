...
export default class MatchStatistics extends UpdateReceiverElement {
  ...
  renderSkirmish(skirmish) {
    const data = MatchStatistics.getTemplateData(skirmish.scores, __('Skirmish') + ' ' + skirmish.id);
    const content = this.statsFunction(data);
    this.skirmishes.insertAdjacentHTML('beforeend', content);
  }

  renderKd(payload) {
    const kills = MatchStatistics.getTemplateData(payload.kills, __('Kills'));
    const killsContent = this.statsFunction(kills);
    this.kd.insertAdjacentHTML('beforeend', killsContent);

    const deaths = MatchStatistics.getTemplateData(payload.deaths, __('Deaths'));
    ...
    const kdRatios = MatchStatistics.getTemplateData(ratios, __('K/D'));
